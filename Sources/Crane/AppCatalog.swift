...
enum Generator: Hashable {
    case none
    case password(Int)
    case base64(Int)
    case uuid

    var isGenerated: Bool { if case .none = self { return false } else { return true } }
    ...
    static func parse(_ raw: String?) -> Generator {
...
struct TemplateVar: Identifiable, Hashable {
    var id: String { key }
    let key: String
    let label: String
    ...
    func initialValue() -> String { generator.isGenerated ? generator.value() : defaultValue }
}

struct TemplateLink: Identifiable, Hashable {
...
struct AppTemplate: Identifiable, Hashable {
    ...
    let logo: String?            // dashboard-icons slug, or a full https URL
    let links: [TemplateLink]
    let tags: [String]
    let primaryPortKey: String?  // which variable holds the host port to "Open" after deploy
    let variables: [TemplateVar]
    let compose: String          // Compose YAML using ${VAR} placeholders
...
enum Secrets {
    ...
    static func token(_ length: Int = 24) -> String {
        String((0..<length).map { _ in alphabet.randomElement()! })
    }
    static func base64(bytes: Int = 32) -> String {
...
enum AppCatalog {
    static let all: [AppTemplate] = loadAll()

    static var byCategory: [(category: AppTemplate.Category, apps: [AppTemplate])] {
    ...
    static func loadAll() -> [AppTemplate] {
