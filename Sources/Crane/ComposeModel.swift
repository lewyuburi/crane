...
struct ComposeBuild: Hashable {
    var context: String        // absolute path to the build context
    var dockerfile: String?    // absolute Dockerfile path
...
struct ComposeProjectRef: Identifiable, Hashable, Codable {
    var id: String { path }
    let path: String          // absolute path to the compose file
    let displayName: String   // shown in the sidebar
    let projectName: String   // sanitized name used for the network + labels
}
...
struct ComposeService: Identifiable, Hashable {
    ...
    var command: [String] = []
    var ports: [String] = []        // "hostPort:containerPort[/proto]"
    ...
    func runImage(project: String) -> String? {
...
enum ComposeParsing {
    ...
    static func parse(yaml: String, baseDir: URL, projectNameOverride: String? = nil) throws -> ComposeProject {
    ...
    static func loadDotEnv(_ url: URL) -> [String: String] {
    ...
    static func interpolate(_ s: String, _ vars: [String: String]) -> String {
        var out = ""
        var i = s.startIndex
        while i < s.endIndex {
