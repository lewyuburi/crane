...
public struct MountPoint: Hashable, Sendable {
...
public struct Container: Identifiable, Hashable, Sendable {
    ...
    public var memoryBytes: Int64? = nil
    ...
    public func recreateArguments(memory: String?, cpus: Int?) -> [String] {
    ...
    public var sortedLabels: [(key: String, value: String)] {
...
public enum ContainerStatus: String, Hashable, Sendable {
    case running
    case stopped
    case created
    case unknown
...
public struct ContainerImage: Identifiable, Hashable, Sendable {
    public let id: String              // image digest id (same image → same id)
    public var name: String            // repository, e.g. docker.io/library/alpine
    public var tags: [String]          // all tags pointing at this image
    public var references: [String]    // full refs (name:tag) for deletion
    public var size: Int64?

    public var displayTags: String { tags.sorted().joined(separator: ", ") }
}
...
public enum ContainerDecoding: Sendable {
    public static func decodeContainers(from data: Data) throws -> [Container] {
        let objects = try topLevelObjects(data)
        return objects.compactMap(container(from:))
    }

    public static func decodeImages(from data: Data) throws -> [ContainerImage] {
