...
public actor StackInstaller {
    ...
    /// it also links into `~/.docker/cli-plugins`, which is the only place `docker compose` looks.
    public func link(_ artifact: Artifact) throws {
        ...
        try replaceSymlink(at: layout.link(for: artifact.component), with: source)
